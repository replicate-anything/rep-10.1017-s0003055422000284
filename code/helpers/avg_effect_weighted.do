/* 
		...
		Modified to accommodate IPWs by Robert A. Blair, Brown University, robert_blair@brown.edu 
*/      

/* Add `aw' as weight denoting probability of assignment to treatment (for treated units) and control (for control units). Used for calculating ATE when assignment probabilities vary by block */
cap program drop avg_effect_weighted_reg
program define avg_effect_weighted_reg, rclass
        syntax varlist(min=2 max=100) [if], x(string) Effectvar(string) Controltest(string) ap(string) [Robust CLuster(string) KEEPmissing]
        version 9.2
        if "`keepmissing'"~="" {
        ...
        quietly {
                ...
                * loop through all y variables
                ...
                foreach var in `varlist' {
                        ...
                        * regress y on x
                        reg `var' `x' [iweight=`ap'] if `marked'
                        est store _ae`nv'
                }
